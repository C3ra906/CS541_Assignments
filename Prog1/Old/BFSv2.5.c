...
Node* addFrontier2(Node ** frontier, Node * to_add, int heuristic)
{
    ...
    if (current == NULL)
    {
        ...
        current = copy_node(to_add, heuristic);
        printf("Returned \n");
        current->next = NULL;
        current->parent = NULL;
       // frontier = &c;
    ...
    else
    {
        ...
        while (current->next != NULL && to_add->f > current->f)
        {
            printf("In while loop \n");
            current = current->next;
        }

        if (current->f < to_add->f)//last node is still bigger in f value than to_add
        {
            current->next = copy_node(to_add, heuristic);
...
Node * expand (Node * current, Node * frontier, Node * reached, int heuristic)//Priority Queue
{
   ...
   if(R != NULL)
   {
       ...
       if(in_reached == -1)
       {
          ...
          frontier = addFrontier2(front, R, heuristic);
   ...
   if (L != NULL)
   {
       ...
       if(in_reached == -1)
       {
          ...
          frontier = addFrontier2(front, L, heuristic);
   ...
   if (U != NULL)
   {
       ...
       if(in_reached == -1)
       {
          ...
          frontier = addFrontier2(front, U, heuristic);
   ...
   if (D != NULL)
   {
       ...
       if(in_reached == -1)
       {
          ...
          frontier = addFrontier2(front, D, heuristic);
...
void BFS(Node * ini, Node * frontier, Node * reached, int heuristic)
{
    Node * current = NULL;
    Node * popped = NULL;
    Node ** front = &frontier;
    //printf("In BFS current popped before while \n");
    //print_state(current);
    while (isEmpty(frontier) == false)
    {
        ...
        //printf("Frontier now should be empty: \n");
        //if (isEmpty(frontier) == true)
        //{
...
int main ()
{
    ...
    Node * current = NULL;//ptr for current node in the search tree?
    ...
    frontier = addFrontier2(front, head, heuristic);
    print_queue(frontier);
    reached = addReached(reached, head, heuristic);
    BFS(head, frontier, reached, heuristic);
    return (EXIT_SUCCESS);
}
