...
typedef struct node{//Node representing each state
    ...
    int d;//keeps track of depth for g(n)
}Node;
...
int calc_f (Node * given, int heuristic, int depth)//calculates f(n) for all 3 heuristics and returns the heuristics asked for
{
    int f = 0;
    int h1 = 0;
    int h2 = 0;
    int h3 = 0;
    int g = depth;//g(n), path cost from initial state to n
...
void fill_initial(Node * given, int num[3][3], int heuristic)//fills out initial 3x3 puzzle state (the starting state)
{
    ...
    given->sol = NULL;
    given->d = 0;
    f = calc_f(given, heuristic, given->d);
    given->f = f;
}

void create_child(Node * given, Node * child, int heuristic, int depth)//creates children of the state
{
    int f = 0;
    int g = 0;
    ...
    g = given->d;
    child->d = g + 1;
    f = calc_f(child, heuristic, child->d);
    child->f = f;
    child->parent = NULL;
    child->next = NULL;
...
Node* addFrontier2(Node ** frontier, Node * to_add, int heuristic)//adds a node to frontier priority queue
{
    ...
    {
        while ((*frontier)->next != NULL && to_add->f > (*frontier)->f)//travel until node is the last node or if value of the node is larger than to_add's
        {
            (*frontier) = (*frontier)->next;
        }

        if ((*frontier)->f < to_add->f)//node pointing to is still bigger in f value than that of to_add
        {
            temp = copy_node(to_add, heuristic);
            temp->next = *frontier;
            temp->parent = (*frontier)->parent;
            if ((*frontier)->parent != NULL)//node pointing to is the head of the list
            {
        ...
        else//to_add f value is smaller than f value of the node pointing to
        {
            temp = copy_node(to_add, heuristic);
            temp->next = *frontier;
            if ((*frontier)->parent == NULL)//node pointing to is the head of the list
...
Node* action_U (Node * current, int row, int col, int heuristic)//Moves b up
{
    ...
    create_child(current, new_state, heuristic, current->d);//creates exact copy of parent for updating into child state
    new_state->parent = current;
    val = new_state->state[row - 1][col];
    new_state->state[row - 1][col] = 9;
    new_state->state[row][col] = val;
    new_state->f = calc_f(new_state, heuristic, new_state->d);
...
Node* action_L (Node * current, int row, int col, int heuristic)//Moves b left
{
    ...
    create_child(current, new_state, heuristic, current->d);//creates exact copy of parent for updating
    new_state->parent = current;
    val = new_state->state[row][col - 1];
    new_state->state[row][col - 1] = 9;
    new_state->state[row][col] = val;
...
Node * expand (Node * current, Node * frontier, Node * reached, int heuristic)//expands current node to create frontier nodes
{
    ...
    int row = row_col[0];
    int col = row_col[1];
    printf("In expand. Current node's depth is %d \n", current->d);
   if(row == 0 && col == 0)//b is at [0][0]
   {
       ...
       printf("created R child. Depth is %d \n", R->d);
       printf("created D child. Depth is %d \n", D->d);
       print_state(R);
       print_state(D);
   }
   else if(row == 0 && col == 1)
   {
       ...
       printf("created R child. Depth is %d \n", R->d);
       printf("created L child. Depth is %d \n", L->d);
       printf("created D child. Depth is %d \n", D->d);
   }
   else if(row == 0 && col == 2)
   {
   ...
   else if(row == 1 && col == 0)
   {
       ...
       printf("created R child. Depth is %d \n", R->d);
       printf("created D child. Depth is %d \n", D->d);
       printf("created U child. Depth is %d \n", U->d);
   }
   else if(row == 1 && col == 1)
   {
       R = action_R(current, row, col, heuristic);
...
int AStar(Node * ini, Node * frontier, Node * reached, int heuristic, Node initial)//A* Search function
{
    Node * current = NULL;
    Node * popped = NULL;
    ...
    while (isEmpty(frontier) == false && counter != 10000)
    {
       ...
       // printf("In while loop for Astar \n");
       ...
       // printf("End of while loop, after frontier \n");
    }
...
int main ()
{
    ...
    int heuristic = 1;//heuristic value chosen. Choices are 1, 2, and 3.
    /*
    ...
    steps = AStar(head, frontier, reached, heuristic, initial);//runs BFS and returns steps BFS took
    ...
    /*printf("Initial State 2 is: 4 1 3 2 5 b 7 8 6\n");
    head = &initial;
    board[0][0] = 8;
    board[0][1] = 1;
    board[0][2] = 3;
    ...
    steps = AStar(head, frontier, reached, heuristic, initial);
    printf("It took %d steps to find the solution \n\n", steps);
    free(frontier);
    free(reached);
    free(head);
    ...
    printf("Initial State 3 is: 1 3 5 7 b 2 8 4 6\n");
    head = &initial;
    board[0][0] = 1;
    board[0][1] = 3;
    board[0][2] = 5;
    ...
    /*printf("Initial State 4 is: 4 1 3 2 5 b 7 8 6\n");
    head = &initial;
    board[0][0] = 7;
    board[0][1] = 2;
    board[0][2] = 4;
