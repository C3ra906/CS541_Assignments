...
void create_child(Node * given, Node * child, int heuristic)//creates children of the state, with updated depth
{
    int f = 0;
    int g = 0;
    ...
    g = given->d;
    child->d = g + 1;
    f = calc_f(child, heuristic, child->f);
    child->f = f;
    child->parent = NULL;
    child->next = NULL;
...
Node* addFrontier3(Node ** frontier, Node * to_add, int heuristic)//adds a node to frontier priority queue
{
    Node * hold = NULL;
    Node * temp = NULL;
...
Node * expand (Node * current, Node * frontier, Node * reached, int heuristic)//expands current node to create frontier nodes
{
   ...
   if(R != NULL)//R exists
   {
       ...
       if(in_reached == -1)//state is not in reached list
       {
          ...
          frontier = addFrontier3(&frontier, R, heuristic);//add to frontier priority queue
       ...
       else if(in_reached > R->f)//state is in reached list but current state has better f(n) value
       {
          ...
          frontier = addFrontier3(&frontier, R, heuristic);
          printf("Added a R child. Frontier: \n");
       }
       print_queue(frontier);
   }
   if (L != NULL)
   {
       ...
       if(in_reached == -1)
       {
          ...
          frontier = addFrontier3(&frontier, L, heuristic);
   ...
   if (U != NULL)
   {
       ...
       if(in_reached == -1)
       {
          ...
          frontier = addFrontier3(&frontier, U, heuristic);
   ...
   if (D != NULL)
   {
       ...
       if(in_reached == -1)
       {
          ...
          frontier = addFrontier3(&frontier, D, heuristic);
...
int main ()
{
    ...
    printf("This program runs A* search on a 8-puzzle.\n");
    /*
    ...
    *//*
    ...
    frontier = addFrontier3(&frontier, head, heuristic);
    reached = addReached(reached, head, heuristic);
    steps = AStar(head, frontier, reached, heuristic, initial);
    printf("It took %d steps to find the solution \n\n", steps);
    free(frontier);
